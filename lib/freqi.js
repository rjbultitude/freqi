(function (global, factory) {
  ...
  * ------------
  */
  function getOctaveMult(interval) {
      var _intervalAbs = Math.abs(interval);
      var _diff = _intervalAbs - TWELVE_TONE;
      var _mult = Math.floor(_intervalAbs / TWELVE_TONE);
      var _newInterval = _diff * (_mult * TWELVE_TONE) - 1;
      if (_diff <= 0) {
          return {
              mult: 1,
  ...
  function getJustIntNote(eTNoteConfig, _up) {
      var _rangeObj = getOctaveMult(eTNoteConfig.interval);
