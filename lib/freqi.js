...
(function (root, factory) {
  ...
  */
  ...
  function getNotesFromIntervals(pConfig) {
      __PROD__ && console.log('pConfig', pConfig);
      var _scaleArray = [];
      // For Inversions
      var _inversionStartNoteIndex = pConfig.inversionStartNote;
      var _newNote;
      for (var i = 0; i < pConfig.loopLength; i++) {
        ...
        //console.log('intervaloffset ' + _inversionStartNoteIndex + ' centreNote Index ' + pConfig.rootNote);
        var finalIndex = pConfig.scaleIntervals[_inversionStartNoteIndex] + pConfig.rootNote;
  ...
  function getFreqs(msConfig) {
      ...
      if (typeof msConfig !== 'object') {
        __PROD__ && console.warn('Musical Scale Config should be an object');
      ...
      __PROD__ && console.log('_intervalsFull', _intervalsFull);
      // Inversions are acheived by
      // selecting an index from within the intervals themselves
      var _loopLength = _intervalsFull.length - _validConfig.inversionStartNote;
      ...
      _scaleArray = getNotesFromIntervals({
          ...
          inversionStartNote: _validConfig.inversionStartNote,
