...
function getModes(tuningSystemsData) {
    var modes = [];
    for (var key in tuningSystemsData) {
...
function getJustIntCommaNote(eTNoteConfig, _up) {
    if (eTNoteConfig.interval === 0) {
        return eTNoteConfig.startFreq;
    }
    var notesInOctave = tuningSystemsData[eTNoteConfig.mode].intervalsInOctave;
    var ratio = tuningSystemsData[eTNoteConfig.mode].intervalRatios;
...
function getJustTuningSystems(tuningSystemsData) {
    ...
    Object.keys(justTuningSystems).forEach(function (key) {
        if (tuningSystemsData[key].type === JUST_STR) {
            Object.defineProperty(justTuningSystems, key, {
...
function getSingleFreq(eTNoteConfig) {
    ...
    var tuningSysType = getTuningSystemType(eTNoteConfig.mode);
